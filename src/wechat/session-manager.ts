/**
 * 微信会话管理器
 * 为每个 (房间, 用户) 对维护独立的 ConversationLoop 实例
 * 处理会话生命周期、消息队列和速率限制
 */
...
export class SessionManager {
   ...
   * 生成会话键
   */
  private getSessionKey(roomId: string | null, userId: string): string {
    return roomId ? `room:${roomId}:user:${userId}` : `dm:${userId}`;
  }
   ...
   */
  private getOrCreateSession(roomId: string | null, userId: string): SessionEntry {
    const key = this.getSessionKey(roomId, userId);
  ...
  async processMessage(
    ...
    const entry = this.getOrCreateSession(roomId, userId);
   ...
   * 重置指定会话
   */
  resetSession(roomId: string | null, userId: string): void {
    const key = this.getSessionKey(roomId, userId);
    this.sessions.delete(key);
  }
