...
export function getToolGuidelines(
  ...
  const items: (string | string[] | null)[] = [
    ...
    toolNames.has('Mcp') ? `MCP-First Rule: When you encounter a task that goes beyond code editing (e.g. sending emails, managing calendars, posting to social media, accessing databases, file conversions), your FIRST action must be to search for a matching MCP tool using the Mcp tool or McpManage. Do NOT jump to Browser, manual workarounds, or asking the user to do it themselves. The sequence is: (1) Search installed MCP tools with Mcp tool, (2) If not found, check disabled MCP servers with McpManage list and enable relevant ones, (3) If still not found, use the tool-discovery skill to search community MCP registries and install one, (4) Only after all MCP options are exhausted, consider alternatives. Many tasks that seem to require browser automation or manual user action can be solved by an MCP server in one tool call.` : null,
  ];
...
export function getCodingGuidelines(toolNames: Set<string>, todoToolName: string, askToolName: string): string {
  ...
  const items: (string | string[])[] = [
    ...
    'Research Before Asking: When the user mentions a name, term, product, or concept you don\'t recognize, ALWAYS search for it first (using WebSearch, WebFetch, or other search tools) before asking the user what it is. A smart assistant Googles first, then discusses. Only after searching and still being unsure, you may ask the user — but bring your search results: "I found X, is that what you mean?" Never ask "What is X?" without searching first.',
    ...
    `When you encounter a task that your current tools cannot handle well (e.g. sending emails, creating PPTs, Excel files, specialized file formats, accessing external services), your FIRST instinct must be to search for an MCP server that can do it. Priority order: (1) Search installed/disabled MCP tools with Mcp tool and McpManage, (2) Use tool-discovery skill to search MCP official registry and Smithery, (3) Web search as last resort. Do NOT attempt browser automation or ask the user to do it manually before exhausting MCP options.`,
