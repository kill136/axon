...
<details>
<summary>桌面应用（Windows / macOS / Linux）</summary>

从 [chatbi.site](https://www.chatbi.site) 下载最新安装包：
...
| 执行 | Bash, Cron (定时任务), 后台任务 |
| Web | WebFetch, WebSearch |
| 代码 | Jupyter Notebook (编辑+写入), LSP 集成 |
| 浏览器 | 基于 Playwright 的全浏览器自动化 |
| 智能体 | 子代理 (Explore, Plan, Guide, Monitor, Parallel) |
| 蓝图 | 任务分解, Lead Agent, Dispatch Workers |
| 规划 | 规划模式, 目标追踪, 任务管理, TodoWrite |
| 记忆 | 长期记忆, embedding 向量, BM25, 混合搜索 |
| 集成 | MCP 协议, 技能市场, 运行时工具创建, 自我进化 |
| 感知 | 摄像头 (Eye), 麦克风 (Ear) |
| 结构化 | 结构化输出, 代码评审提交 |
...
Bug 反馈和功能建议请提交 [Issue](https://github.com/kill136/axon/issues)。
